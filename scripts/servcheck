...
if __name__ == '__main__':
    ...
    if not os.path.exists(cfgfile):
        print >> sys.stderr, "File %s does not exist." % cfgfile
        sys.exit(os.EX_IOERR)

    try:
        cp = config.read_ini(cfgfile)
    except:
        print >> sys.stderr, "Opening the file raised the following error."
        sys.exit(os.EX_IOERR)
    ...
    if args.catalog:
        ...
        try:
            ...
            print >> sys.stderr, "Catalog '%s' could not be found inside config file."
            sys.exit(os.EX_IOERR)
        caturls[cat] = url
    else:
        for cat in cp.keys():
    ...
    status = main(caturls.values(),args.verification)
    if not status:
        sys.exit(os.EX_NOHOST)
    else:
        sys.exit(os.EX_OK)
